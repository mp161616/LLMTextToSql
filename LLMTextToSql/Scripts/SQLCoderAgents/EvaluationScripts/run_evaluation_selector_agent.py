...
def load_settings(path="../appsettings.json"):
    with open(path, "r", encoding="utf-8") as f:
        config = json.load(f)
    return config["Paths"]["SqlCoderSelectorGeneratedQueries"], config["Paths"]["SqlCoderSelectorEvaluationResults"]
...
# PATHS
selector_generated_queries, selector_evaluation_results = load_settings()
EVALUATION_FILE = selector_evaluation_results
RESULTS_PATH = Path(selector_generated_queries)
