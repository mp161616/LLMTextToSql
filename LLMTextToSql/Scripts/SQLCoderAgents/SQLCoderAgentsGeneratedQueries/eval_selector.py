...
def load_settings(path="../appsettings.json"):
    with open(path, "r", encoding="utf-8") as f:
        config = json.load(f)
    return config["Paths"]["SqlCoderSelectorGeneratedQueries"]
...
for idx, test in enumerate(test_cases):
    ...
    try:
        output = subprocess.check_output(
            ["python", "SQLCoderAgents/Agents/selector.py", question],
...
with open("output/selector_eval_results01.json", "w", encoding="utf-8") as f:
