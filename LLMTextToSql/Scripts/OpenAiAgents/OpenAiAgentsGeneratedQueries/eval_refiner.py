...
def load_settings(path="../appsettings.json"):
    with open(path, "r", encoding="utf-8") as f:
        config = json.load(f)
    return config["Paths"]["OpenAISelectorGeneratedQueries"], config["Paths"]["OpenAIRefinerGeneratedQueries"]
...
for test in selector_results:
    ...
    try:
        full_output = subprocess.check_output(
           [
              "python", "OpenAiAgents/Agents/refiner.py",
              selector_output,
         ...
         # Get the last non-empty line (OpenAi's final SQL output)
